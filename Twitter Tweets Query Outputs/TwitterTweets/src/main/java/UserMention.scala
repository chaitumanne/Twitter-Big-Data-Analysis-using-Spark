  ...
  object UserMention {
    def main(args: Array[String]) {
      ...
      val movie = sqlContext.sql("SELECT user.entities.user_mentions, count(*) as c FROM EntertainmentTable group by user.entities.user_mentions order by c desc limit 10")
